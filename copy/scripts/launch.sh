...
geth --preload "/private-ethereum/scripts/miner.js" --port ${ETH_PORT} --networkid 1234 --datadir=./datadir --nodiscover  --miner.gasprice=0  --http --http.port ${ETH_HTTP_PORT} --http.addr ${ETH_HTTP_DNS}  --rpcapi "eth,net,web3,personal,miner,admin" --http.corsdomain "*" --allow-insecure-unlock --unlock ${adress} --password list.txt --mine 
